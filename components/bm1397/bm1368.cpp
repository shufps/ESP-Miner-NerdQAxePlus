...
uint8_t BM1368::init(uint64_t frequency, uint16_t asic_count, uint32_t difficulty)
{
    ...
    // Analog Mux Control
    this->send6(CMD_WRITE_ALL, 0x00, 0x54, 0x00, 0x00, 0x00, 0x03);

    // Set the IO Driver Strength on chip 00
    this->send6(CMD_WRITE_ALL, 0x00, 0x58, 0x02, 0x11, 0x11, 0x11);
    ...
    for (uint8_t i = 0; i < chip_counter; i++) {
        ...
        // Core Register Control
        this->send6(CMD_WRITE_SINGLE, i * 2, 0x3C, 0x80, 0x00, 0x80, 0x18);
        // Core Register Control
