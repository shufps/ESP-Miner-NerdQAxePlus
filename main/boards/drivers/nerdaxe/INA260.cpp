...
float INA260_read_current(void)
{
    ...
    esp_err_t err = i2c_master_register_read(INA260_I2CADDR_DEFAULT, INA260_REG_CURRENT, data, 2);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "error reading current");
        return 0.0f;
    }
...
float INA260_read_voltage(void)
{
    ...
    esp_err_t err = i2c_master_register_read(INA260_I2CADDR_DEFAULT, INA260_REG_BUSVOLTAGE, data, 2);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "error reading voltage");
        return 0.0f;
    }
...
float INA260_read_power(void)
{
    ...
    esp_err_t err = i2c_master_register_read(INA260_I2CADDR_DEFAULT, INA260_REG_POWER, data, 2);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "error reading power");
        return 0.0f;
    }
