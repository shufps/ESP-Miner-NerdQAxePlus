...
esp_err_t rest_common_get_handler(httpd_req_t *req)
{
    ...
    if (fd < 0) {
        ...
        if (errno == ENOENT) {
            // in case we request something that doesn't exist we
            // redirect to the portal with ESP_OK
